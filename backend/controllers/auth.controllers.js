...
export const signUp = async (req, res) => {
  try {
    ...
    if (password.length < 6) {
      return res
        .status(400)
        .json({ message: "password must be alleast 6 characters" });
    }
...
export const signIn = async (req, res) => {
  try {
    ...
    if (!isMatch) {
      return res
        .status(400)
        .json({ message: "password not matched/Incorrect Password" });
    }
...
export const verifyOtp = async (req, res) => {
  try {
    ...
    if (!user || user.resetOtp !== otp || user.otpExpires < Date.now()) {
      return res.status(400).jaon({ message: "invlaid/expired otp" });
    }
