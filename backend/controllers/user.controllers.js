...
export const getCurrentUser = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId).populate(
      "posts loops story  posts.author posts.comments story following",
    );
    if (!user) {
...
export const follow = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: `followerror ${error}` });
  }
};
...
export const getAllNotifications = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: `get notofication error ${error}` });
  }
};
